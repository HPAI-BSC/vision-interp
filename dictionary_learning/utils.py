...
def load_dataset_from_yaml(dataset_name, split=None):
    """Load a dataset configured in ``config/dataset_config.yaml``.

    For Hub datasets (``path: null``), always pass ``split`` (or rely on
    ``eval_split`` in the YAML when ``split`` is omitted) so only that split is
    downloaded. Calling ``load_dataset`` without a split fetches every split.
    """
    ...
    if isinstance(yaml_path, str) and os.path.isdir(yaml_path):
        ...
        hub_split = split if split is not None else ds_entry.get("eval_split")
        if hub_split is not None:
...
def hf_dataset_to_generator(dataset_name, split="train", ratio_of_training_data=1):

    dataset, column_names_dict = load_dataset_from_yaml(dataset_name, split=split)
    ...
    if text_column_name is not None and image_column_name is not None:
        ...
        def gen_text_and_images():
            for x in iter(ds_split):
                yield {"text": [x[text_column_name]], "image": [x[image_column_name]]}
        return gen_text_and_images(), len(ds_split)
        ...
        return gen_images(), len(ds_split)
