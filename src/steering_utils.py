...
def read_top_k_images(ds, latent_id, top_k_images_dir, k=1, top_k_random_sample=False, partition=None, seed=42):
    ...
    images = [ds[img_id]['image'] for img_id in top_ids]
    ...
    if top_k_random_sample:
        ...
        random_indices = rng.choice(np.arange(0, len(heatmaps)), size=k, replace=False)
        ...
        images = [ds[img_id]['image'].convert("RGB") for img_id in top_ids]
