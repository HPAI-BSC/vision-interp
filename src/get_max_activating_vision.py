...
import numpy as np
from utils.sae_utils import get_patch_sae_codes, resolve_sae_path_from_hub_name
from utils.hf_hook_utils import get_module_output
from utils.utils import get_paths, load_active_latents
...
def subset_max_activating_vision(generator, model, tokenizer, processor, cfg):
    ...
    for _ in range(cfg.subset_n_batches):
        ...
        batch_layer_out = get_module_output(model, inputs, tokenizer, cfg, processor)
...
def get_top_ids_values_sae_latents(generator, model, tokenizer, processor, latents_to_consider, sae, cfg):
    ...
    for batch_idx in tqdm(range(total_batches)):
        ...
        # # Create a mask for valid positions (at least 3 spatial values > 0 across any latent)
        # # Shape: [subset_size, n_latents]
        # valid_mask = (subset_codes > 0).sum(dim=(1, 2)) >= 3

        # # Zero out invalid entries to avoid contamination
        # #subset_codes = subset_codes.clone()  # in case you need original later
        # subset_codes[~valid_mask.unsqueeze(1).unsqueeze(2).expand_as(subset_codes)] = 0


        ############################################################
        # New: accumulate all activations and indices for each latent
        # For aggregation, we need to flatten the activations for each latent across all images in the batch
        # We'll use the aggregation function (mean or max) over spatial dims if needed
        # subset_codes: [subset_size, height, width, n_latents]
        # We'll aggregate over height and width
    ...
    if cfg.ids_selection == "partition":
        ...
        for latent_id in latents_to_consider:
            ...
            # We create a list of empty lists to store the heatmaps for each latent
...
def main(cfg):
    ...
    sae_path = resolve_sae_path_from_hub_name(
        cfg.sae_name, DATA_DIR, hf_org=cfg.hf_sae_org
    )
    print(f"Using SAE at {sae_path}")
    ...
    generator, len_dataset = hf_dataset_to_generator(dataset, split=cfg.dataset_split)
    if cfg.n_images == "all":
    ...
    if cfg.dataset_split == "test":
        cfg.n_images = min(cfg.n_images, 50000)
        print(f'Test split: capping to {cfg.n_images} images')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="javifer",
        help="Hugging Face organization/user hosting the SAE collection (repo = {org}/<first path segment>).",
    )
    ...
    parser.add_argument("--ids_selection", type=str, default="top_k", choices=["partition", "top_k"])
...
python -m src.get_max_activating_vision \
        --top_k 10 \
        --ids_selection top_k \
        --sae_name google_gemma-3-4b-it-saes/enc_res_out_layer_16_top_k_8192_25_1.0_HF_22191080 \
        --n_images 1000
